/**
 * Unified Seed Script for AgentWorks
 *
 * This script creates all necessary data for both local and production environments:
 * 1. Platform configuration (plans, providers, agents, settings)
 * 2. Admin user: admin@agentworksstudio.com / AdminPass123!
 * 3. Web user: thomas@thomasbedell.com / Password123!
 *
 * Usage:
 *   npx tsx prisma/seed-unified.ts
 *   pnpm db:seed
 */
...
const founderPlans = [
  {
    ...
    totalSpots: 50,
    remainingSpots: 50,
    features: ['Lifetime Pro access', '50% discount forever', 'Exclusive Discord channel', 'Priority support', 'Early access to features'],
    affiliateBonus: 20,
    active: true,
  },
  {
    ...
    totalSpots: 100,
    remainingSpots: 100,
    features: ['Lifetime Starter access', '30% discount forever', 'Exclusive Discord channel', 'Early access to features'],
    affiliateBonus: 10,
    active: true,
  },
  {
    ...
    totalSpots: 200,
    remainingSpots: 200,
    features: ['3 months free', '20% discount for first year', 'Exclusive Discord channel'],
    affiliateBonus: 5,
    active: true,
  },
];
...
const WEB_USER = {
  ...
  tenantName: "Thomas Bedell's Workspace",
  tenantSlug: 'thomas-bedell-workspace',
  workspaceName: "Thomas Bedell's Workspace",
};

const ADMIN_USER = {
  email: 'admin@agentworksstudio.com',
  name: 'AgentWorks Admin',
...
async function main() {
  ...
  // ==========================================
  // Step 2: Seed Provider Configurations
  // ==========================================
  console.log('Seeding provider configs...');
  ...
  // ==========================================
  // Step 7: Seed Founder Plans
  // ==========================================
  console.log('Seeding founder plans...');
  for (const fp of founderPlans) {
  ...
  console.log(`  Created ${founderPlans.length} founder plans`);
  ...
  if (existingAdmin) {
    ...
    console.log(`  Updated existing admin: ${ADMIN_USER.email}`);
  } else {
    const adminPasswordHash = await argon2.hash(ADMIN_USER.password);
  ...
  // ==========================================
  // Step 9: Create Web User with Tenant
  // ==========================================
  console.log('\nCreating web user...');
  ...
  const existingUser = await prisma.user.findUnique({
    where: { email: WEB_USER.email },
  });

  if (existingUser) {
    // Update password to ensure consistency
    const userPasswordHash = await argon2.hash(WEB_USER.password);
    ...
    console.log(`  Updated existing user: ${WEB_USER.email}`);
  } else {
    // Create user with tenant, workspace, and settings in a transaction
    const userPasswordHash = await argon2.hash(WEB_USER.password);
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const tenant = await tx.tenant.create({
        data: {
          name: WEB_USER.tenantName,
          slug: WEB_USER.tenantSlug,
          status: 'trial',
    ...
    console.log(`  Created user: ${WEB_USER.email}`);
    console.log(`  Created tenant: ${result.tenant.name}`);
    console.log(`  Created workspace: ${result.workspace.name}`);
  }
  ...
  console.log('========================================\n');
  console.log('Web App Login:');
  console.log(`  Email: ${WEB_USER.email}`);
  console.log(`  Password: ${WEB_USER.password}`);
  console.log(`  URL: http://localhost:5173 (local) or https://app.agentworksstudio.com (prod)\n`);
  console.log('Admin Portal Login:');
  console.log(`  Email: ${ADMIN_USER.email}`);
  console.log(`  Password: ${ADMIN_USER.password}`);
  console.log(`  URL: http://localhost:3020 (local) or https://admin.agentworksstudio.com (prod)\n`);
}
